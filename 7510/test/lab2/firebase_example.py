...
response = db.child('Contacts/bobcATcdeDOTcom').get()
key = response.key()
value = response.val()
print(key)
print(value)
# storage test
storage.child('testing/myfile.txt').put('myfile.txt')

receiver = 'alicewATabcDOTnet'
data = dict(
    subject='Greeting',
...
response = db.child(f'/Messages/{receiver}').push(data)
print(response)

response = db.child('/Contacts/bobcATcdeDOTcom/phone').set(56473829)
print(response)

data = dict(name='Edon Tsang', gender='F',
            phone=98765432, email='et@space.net')
response = db.child('/Contacts/etATspaceDOTnet' + key).set(data)
print(response)
